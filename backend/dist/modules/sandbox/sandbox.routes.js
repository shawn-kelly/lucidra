...
router.get('/dashboard', async (req, res) => {
    try {
        ...
        const progress = sandbox_service_1.SandboxService.getUserProgress(sessionId);
        const missions = sandbox_service_1.SandboxService.getUserMissions(sessionId);
        const advisors = sandbox_service_1.SandboxService.getAvailableAdvisors();
        res.json({
            success: true,
            data: {
                progress,
                missions,
                advisors,
                availableBadges: sandbox_service_1.AVAILABLE_BADGES
            }
        });
    }
    catch (error) {
...
router.post('/missions', async (req, res) => {
    try {
        ...
        const mission = sandbox_service_1.SandboxService.createWorkflowMission(sessionId, title, description, challenge, category || 'custom');
        res.json({
            success: true,
            data: mission
        });
...
router.get('/missions/:missionId', async (req, res) => {
    try {
        ...
        const mission = sandbox_service_1.SandboxService.getMissionById(missionId);
        if (!mission) {
...
router.post('/missions/:missionId/subtasks', async (req, res) => {
    try {
        ...
        const subtask = sandbox_service_1.SandboxService.addSubtaskToMission(missionId, {
            title,
            description,
            assignedAdvisor,
            promptTemplate,
...
router.put('/missions/:missionId/subtasks/:subtaskId/advisor', async (req, res) => {
    try {
        ...
        sandbox_service_1.SandboxService.assignAdvisorToSubtask(missionId, subtaskId, advisorId);
        res.json({
            success: true,
...
router.post('/missions/:missionId/subtasks/:subtaskId/iterations', async (req, res) => {
    try {
        ...
        const iteration = sandbox_service_1.SandboxService.addIterationToSubtask(missionId, subtaskId, promptUsed, advisorResponse, userAnnotation);
        res.json({
            success: true,
            data: iteration
        });
...
router.get('/badges', async (req, res) => {
    try {
        const badges = sandbox_service_1.SandboxService.getAvailableBadges();
        res.json({
            success: true,
            data: badges
        });
