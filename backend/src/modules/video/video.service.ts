...
export async function generateVideo(options: VideoGenerationOptions): Promise<VideoStatus> {
...
async function generateVideoAsync(options: VideoGenerationOptions): Promise<void> {
  const { videoId, content, voiceoverScript, duration } = options;
...
function generateScenesFromContent(content: any): any[] {
  const scenes = [];
  ...
  if (content.highlights) {
    content.highlights.forEach((highlight: string, index: number) => {
      scenes.push({
        id: `scene_${index}`,
        text: highlight,
        duration: 5,
...
function getSceneColor(index: number): string {
  const colors = ['#1FE0C4', '#1C1F26', '#6C75F8', '#FF6B6B', '#00D4AA'];
  return colors[index % colors.length];
}

async function generateVoiceover(videoId: string, script: string): Promise<string> {
...
async function tryAIVideoGeneration(videoId: string, content: any, voiceoverScript?: string): Promise<string | null> {
  try {
...
async function generateWithRunwayML(videoId: string, content: any, voiceoverScript?: string): Promise<string | null> {
  ...
  try {
    ...
    console.error('Runway ML error:', (error as any).response?.data || (error instanceof Error ? error.message : String(error)));
    return null;
  }
}

async function generateWithStabilityAI(videoId: string, content: any, voiceoverScript?: string): Promise<string | null> {
  ...
  try {
    ...
    const response = await axios.post('https://api.stability.ai/v2beta/stable-video/generate', {
      prompt,
      duration: 'short',
      aspect_ratio: '16:9'
    }, {
    ...
    console.error('Stability AI error:', (error as any).response?.data || (error instanceof Error ? error.message : String(error)));
    return null;
  }
}

async function generateWithOpenAI(videoId: string, content: any, voiceoverScript?: string): Promise<string | null> {
  ...
  try {
    ...
    console.error('OpenAI error:', (error as any).response?.data || (error instanceof Error ? error.message : String(error)));
    return null;
  }
}

async function generateWithLeonardoAI(videoId: string, content: any, voiceoverScript?: string): Promise<string | null> {
  ...
  try {
    ...
    const response = await axios.post('https://cloud.leonardo.ai/api/rest/v1/generations-motion', {
      prompt,
      num_images: 1,
    ...
    console.error('Leonardo AI error:', (error as any).response?.data || (error instanceof Error ? error.message : String(error)));
    return null;
  }
}

function createVideoPromptFromContent(content: any): string {
...
function createImagePromptFromContent(content: any): string {
...
async function createSlideshowFromImages(videoId: string, images: any[], voiceoverScript?: string): Promise<string> {
  const videoPath = `/tmp/lucidra-videos/${videoId}.mp4`;
...
async function pollLeonardoJob(videoId: string, jobId: string, apiKey: string): Promise<string | null> {
  const maxAttempts = 30;
  let attempts = 0;
  
  while (attempts < maxAttempts) {
    try {
      const response = await axios.get(`https://cloud.leonardo.ai/api/rest/v1/generations/${jobId}`, {
        headers: {
          'Authorization': `Bearer ${apiKey}`
        }
      });
...
function updateVideoStatus(videoId: string, updates: Partial<VideoStatus>): void {
...
export async function generateStrategicDemo(demoType: string, content: any): Promise<VideoStatus> {
...
function generateStrategicVoiceover(demoType: string, content: any): string {
